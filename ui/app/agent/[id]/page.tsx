...
const formatLinks = (text: string) => {
  ...
  return parts.map((part, i) => {
    ...
    if (matches[i]) {
      ...
      const url = cleanUrl.startsWith("www.")
        ? `https://${cleanUrl}`
        : cleanUrl;
      ...
      try {
        ...
        displayText =
          cleanUrl.length > 30 ? cleanUrl.substring(0, 30) + "..." : cleanUrl;
      }
      ...
      return (
        <div key={index} className="flex gap-2 items-start ml-2 mt-1">
          ...
          <span className="break-words">
            {formatLinks(trimmedLine.substring(1).trim())}
          </span>
        </div>
      );
    }
...
const getSocialLinks = async (agent: Agent) => {
  ...
  if (
    ...
    agent.secrets?.TELEGRAM_BOT_TOKEN
  ) {
    const botDetails = await getTelegramBotDetails(
      agent.secrets.TELEGRAM_BOT_TOKEN
    );
    if (botDetails) {
      links.push({
...
export default function AgentPage() {
  ...
  const [socialLinks, setSocialLinks] = useState<
    Array<{ type: string; url: string; enabled: boolean; username?: string }>
  >([]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_ELIZA_BACKEND_URL}/${agent.agentId}/message`,
        {
          method: "POST",
          headers: {
      ...
      if (data && data.length > 0) {
        data.forEach((response) => {
          const agentMessage: ChatMessage = {
            ...
            role: "assistant",
            expression:
              response.action === "WEB_SEARCH" ? "Searching the web..." : null,
            timestamp: new Date().toISOString(),
          };
  ...
  // Update the renderMessage function to use GIF avatar
  const renderMessage = (msg: ChatMessage, index: number) => (
    ...
    >
      ...
      >
        ...
        {msg.expression && (
          <div className={`text-xs mt-2 italic text-[#f78da7]`}>
            *{msg.expression}*
          </div>
        )}
      </div>
      {msg.role === "user" && (
        <div className="w-8 h-8 rounded-full bg-black overflow-hidden flex-shrink-0 flex items-center justify-center">
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl ">
      <Card className=" border-zinc-800 p-6 mb-6 bg-[#000000]">
                  ...
                  >
                    ...
                    {link.type === "telegram" && (
                      <>
                        ...
                        <span className="text-[#0088cc] hover:text-[#0077b3]">
                          {link.username}
                        </span>
                      </>
                    )}
        ...
        <div className="grid gap-6">
          <Card className="bg-[#000000] border-[#f78da7] border-2 rounded-xl shadow-lg overflow-hidden">
            <div className="p-4 border-b border-[#f78da7] bg-black flex items-center">
              ...
              <div>
                <h3 className="font-semibold text-zinc-400 flex items-center">
              ...
              <div className="ml-auto flex space-x-1">
                <div className="w-2 h-2 rounded-full bg-[#f78da7] animate-pulse"></div>
                <div className="w-2 h-2 rounded-full bg-white animate-pulse delay-100"></div>
                <div className="w-2 h-2 rounded-full bg-[#f78da7] animate-pulse delay-200"></div>
              </div>
            </div>

            <div
              className="p-4 h-[400px] overflow-y-auto space-y-4 bg-zinc-900"
              ref={chatContainerRef}
              style={{
                backgroundImage:
                  "url(\"data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M30,20 Q35,5 40,20 L37,25 Q35,28 33,25 Z' fill='%23f78da7' fill-opacity='0.05'/%3E%3C/svg%3E\")",
            ...
            >
              ...
              {isTyping && (
                <div className="flex gap-2">
                  <div className="w-8 h-8 rounded-full bg-[#f78da7] overflow-hidden flex-shrink-0">
                    <img
                      src="https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExcW5jbzcyMnhleHpteWt0ZWR1eXduNDhoMXB3Nnk4ZTZ3eXZxcm81ayZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/SdvbTgJsHmsZa/giphy.gif"
                      alt="AI Avatar"
                      className="w-full h-full object-cover"
                  ...
                  <div className="rounded-2xl p-3 text-sm bg-white text-black border border-[#f78da7] shadow-md relative">
                    <div className="absolute left-[-5px] top-2 w-3 h-3 rotate-45 bg-white border-l border-b border-[#f78da7]"></div>
                    <div className="flex space-x-1">
                      <div className="w-2 h-2 rounded-full bg-[#f78da7] animate-bounce"></div>
                      <div className="w-2 h-2 rounded-full bg-[#f78da7] animate-bounce delay-100"></div>
                      <div className="w-2 h-2 rounded-full bg-[#f78da7] animate-bounce delay-200"></div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <form
              ...
              className="p-4 border-t border-[#f78da7]/20 bg-zinc-900"
            >
              <div className="flex gap-2 items-center">
                <div className="relative w-full">
                  <Input
                    placeholder={
                      address
                        ? "Nya~ Type your message..."
                        : "Connect wallet to chat with me, nya~"
                    }
                    value={message}
                    onChange={(e) => setMessage(e.target.value)}
                    className="bg-zinc-800 border-[#f78da7]/30 focus:border-[#f78da7] pr-10 rounded-full pl-4"
                    disabled={isSubmitting || !address}
                  />
                  <div className="absolute right-3 top-2.5 text-[#f78da7] text-sm opacity-70">
                    ^ω^
                  </div>
                </div>
                <Button
                  type="submit"
                  size="icon"
                  className="bg-[#f78da7] hover:bg-[#f78da7]/80 rounded-full w-10 h-10 flex items-center justify-center shadow-lg overflow-hidden relative group transition-all duration-300 hover:scale-110 hover:shadow-xl"
                  disabled={isSubmitting || !address}
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-pink-400 to-[#f78da7] opacity-100 group-hover:opacity-90 transition-opacity"></div>
                  <div className="relative z-10 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3">
                    ...
                    >
                      <path
                        d="M30 20Q35 5 40 20L37 25Q35 28 33 25L30 20Z"
                        fill="white"
                      />
                      <path
                        d="M60 20Q65 5 70 20L67 25Q65 28 63 25L60 20Z"
                        fill="white"
                      />
                      <path
                        d="M15 40Q20 25 25 40L22 45Q20 48 18 45L15 40Z"
                        fill="white"
                      />
                      <path
                        d="M75 40Q80 25 85 40L82 45Q80 48 78 45L75 40Z"
                        fill="white"
                      />
                      <path
                        d="M35 65Q50 40 65 65Q72 75 65 85Q58 92 50 90Q42 92 35 85Q28 75 35 65Z"
                        fill="white"
                      />
                    </svg>
                  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        >
          ...
          <p className="text-sm text-zinc-400 mb-4">
            Connect your agent with Twitter
          </p>
          {agent.twitter ? (
            <span className="text-[#f78da7] text-sm flex items-center gap-1">
              <Check className="w-4 h-4" /> Connected
        ...
        <Card className="bg-[#000000] border-zinc-800 p-6 flex flex-col items-center text-center hover:border-[#f78da7]/50 transition-colors cursor-pointer">
          <div className="w-12 h-12 bg-[#000000] rounded-full flex items-center justify-center mb-4 border border-zinc-800">
